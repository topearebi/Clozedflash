...
// 1. DATABASE (Dexie)
// ==========================================
const db = new Dexie('FlashDeckDB_v7');
db.version(1).stores({
    cards: '++id, type, tag, status, dueDate',
    chapters: '++id, title, tag',
    audio: '++id, cardId, segmentId'
});

// ==========================================
// 2. LOGIC & GAMIFICATION SERVICE
// ==========================================
const Logic = {
    // State
    gamification: JSON.parse(localStorage.getItem('fd7_gamification')) || {
        streak: 0,
        lastStudyDate: null,
        xp: 0,
        level: 1,
    ...
    dailyStats: JSON.parse(localStorage.getItem('fd7_daily')) || {
    ...
    checkDailyReset() {
        ...
        if (last !== today) {
            ...
            if (last !== yesterday && last !== null) {
                this.gamification.streak = 0; // Reset streak
            }
            // If it's a new day, give login bonus
            if (last !== today) {
                this.addSparks(50);
    ...
    recordAction(type) {
        if (type === 'REVIEW') {
            ...
            this.addXP(10);
            if(this.gamification.streak === 0) this.gamification.streak = 1; // Start streak
        }
        if (type === 'LEARN') {
...
const AudioService = {
    ...
    async startRecording() {
        ...
        try {
            ...
            this.recorder.start();
            return true;
        } catch (e) { console.error(e); alert("Mic Error."); return false; }
    },
...
const Dashboard = {
    async refresh() {
        ...
        const masteredCount = await db.cards.where('interval').above(21).count(); // Mastered logic
        ...
        document.getElementById('home-activity-list').innerHTML = recent.map(c => `
            <div class="chapter-item" onclick="Reader.open(${c.id})">
                ...
                <span class="item-sub">${c.segments.length} lines</span>
            </div>
        `).join('');
    }
};
...
const Importer = {
    async runCardImport() {
        ...
        alert(`Imported ${count} cards to New Queue!`);
        document.getElementById('modal-import').classList.add('hidden');
        Logic.addXP(count * 5); // XP for importing
        Dashboard.refresh();
    },

    async runChapterImport() {
        const title = document.getElementById('imp-chap-title').value;
        const tag = document.getElementById('imp-chap-tag').value;
        const raw = document.getElementById('imp-chap-data').value;
        ...
        alert(`Chapter "${title}" created with ${segments.length} sentences.`);
        document.getElementById('modal-import-chapter').classList.add('hidden');
        Logic.addXP(50); // XP for Chapter
        Library.refresh();
    }
};
...
const StudySession = {
    ...
    async loadNext() {
        ...
        if (this.current.type === 'SENTENCE') this.renderCloze();
            ...
            if (Math.random() > 0.3) this.renderRecog(); else this.renderRecall();
        }
    },
    ...
    async rate(rating) {
        ...
        // Track Stats
        if(this.current.dueDate === null) Logic.recordAction('LEARN');
        else Logic.recordAction('REVIEW');
...
// ==========================================
// 8. READER, STREAM, DECK (Condensed)
// ==========================================
const Reader = {
...
const Deck = {
    async refresh() {
        ...
        document.getElementById('card-list').innerHTML = list.map(c => `<div class="card-item"><div><span class="item-main">${c.target}</span><br><span class="item-sub">${c.native}</span></div></div>`).join('');
    }
};

const Settings = {
    refresh() {
        document.getElementById('setting-xp').textContent = Logic.gamification.xp;
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-backup').onclick = async () => {
        // Simple JSON backup for now (Full Zip logic requires reading all Blobs which is heavy for MVP)
        const allCards = await db.cards.toArray();
        const allChapters = await db.chapters.toArray();
        const data = JSON.stringify({ cards: allCards, chapters: allChapters });
...
// MISSING STREAM BUILDER LOGIC from previous paste (Re-integrating for completeness)
const StreamBuilder = {
    ...
    async handleFileSelect() { const f = document.getElementById('dock-file'); this.tempBlob = await AudioService.handleFileUpload(f); },
    addSegment() {
        ...
        div.innerHTML = `<div class="segment-target">${target}</div><div class="segment-native">${native}</div>`;
        document.getElementById('builder-stream').appendChild(div);
        this.tempBlob = null; document.getElementById('dock-target').value = '';
    },
    async save() {
