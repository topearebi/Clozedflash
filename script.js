...
const Deck = {
    async refresh() {
        ...
        let list;
        if(q) {
            list = await db.cards.filter(c => c.target.toLowerCase().includes(q) || c.native.toLowerCase().includes(q)).toArray();
        } else {
            list = await db.cards.limit(50).toArray(); 
        }
        ...
        document.getElementById('card-list').innerHTML = list.map(c => `
            <div class="card-item">
                ...
                <div><button class="small-btn" onclick="Deck.delete(${c.id})">🗑</button></div>
            </div>
        `).join('');
    },
...
document.addEventListener('DOMContentLoaded', () => {
    Logic.checkDailyReset(); UI.updateStats(); Dashboard.refresh();
    ...
    document.getElementById('btn-quit-game').onclick = () => UI.toggleOverlay('view-game', false); // Fix for Game Exit
    ...
    // Game (Speed Round Logic Missing in previous snippet re-added here)
    const Game = {
        ...
        endRound() {
            if (this.score >= 8) { if(confirm("Next Round?")) { this.round++; this.runRound(); } else UI.toggleOverlay('view-game', false); }
    ...
    document.getElementById('btn-start-cram').onclick = () => {
        ...
        StudySession.start('NEW', 50); // Fallback logic for Cram
    };
    ...
    document.getElementById('btn-backup').onclick = async () => {
        ...
        // In full version, loop db.audio and add blobs to zip folder
